...
impl AppState {
    ...
    pub async fn fetch_chat(&self, ws_id: u64) -> Result<Vec<Chat>, AppError> {
...
mod tests {
    ...
    async fn chat_fetch_all_should_work() -> Result<()> {
        let (_tdb, state) = AppState::new_for_test().await?;
        let chats = state.fetch_chat(1).await.expect("fetch all chats failed");
